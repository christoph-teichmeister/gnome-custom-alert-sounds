...
const AlertSoundToggle = GObject.registerClass(
  class AlertSoundToggle extends QuickMenuToggle {
    _init(manager) {
      ...
      const openFolderItem = new PopupMenu.PopupMenuItem(
    ...
    _refreshCustom() {
      for (const [id] of this._items) {
        if (id.startsWith("custom:")) this._items.delete(id);
      }
    ...
    _updateCurrent() {
      ...
      for (const [id, item] of this._items) {
        ...
        item.setOrnament(
          isSelected ? PopupMenu.Ornament.DOT : PopupMenu.Ornament.NONE,
        );
      }
